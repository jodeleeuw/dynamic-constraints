...
run_PARSER_params <- function(model, seq, condition) {
  ...
  target_weight <- ct$eval("PARSER.getWordStrength('ABC')")
  foil_weight <- ct$eval("PARSER.getWordStrength('DHL')")
...
run_parser_reps <- function(w){
  ...
  for(i in 1:reps_per_condition){
    ...
    runs[[counter]] <- run_PARSER_params('models/parser.js',four_seq, 'four')
    counter <- counter + 1
    runs[[counter]] <- run_PARSER_params('models/parser.js',one_seq, 'one')
    counter <- counter + 1
  }
...
for(i in seq(from=0,to=0.15,by=0.005)){
...
run_mod_parser_reps <- function(w){
  ...
  for(i in 1:reps_per_condition){
    ...
    runs[[counter]] <- run_PARSER_params('models/parser-modified.js',four_seq, 'four')
    counter <- counter + 1
    runs[[counter]] <- run_PARSER_params('models/parser-modified.js',one_seq, 'one')
    counter <- counter + 1
  }
...
for(i in seq(from=0,to=0.075,by=0.005)){
  cat(paste('\rcurrent param value:',i))
...
save(sim_exp, sim_exp_modified_model, file="data/output/PARSER-variations.Rdata")
