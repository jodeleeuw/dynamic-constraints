...
MDLChunker = (function(){
  ...
  module.getLexicon = function() {
    ...
    for(var l in lexicon){
      t.push({
        word: get_canonical_definition(lexicon[l]),
        codeLength: lexicon[l].codeLength
      });
    }
  ...
  */
  ...
  function update_codelengths(){
    ...
    for(l in lexicon){
      ...
      var bitlength = -(Math.log(count/total_length)/Math.log(2));
  ...
  function description_length_increase(newchunk, vector){
    ...
    var L1 = newchunk.count * (log2((N + 3 - CiCj)/(CiCj + 1)) - log2(N/Ci) - log2(N/Cj));
    var L2 = (Ci - CiCj) * (log2((N + 3 - CiCj)/(Ci - CiCj + 1)) - log2(N/Ci)) + (Cj - CiCj) * (log2((N + 3 - CiCj)/(Cj - CiCj + 1)) - log2(N/Cj));
    var L3 = log2( Math.pow(N+3-CiCj,3) / ((CiCj + 1)*(Ci - CiCj + 1)*(Cj-CiCj+1)))
    var L4 = (N - Ci - Cj) * log2( (N + 3 - CiCj)/N )
