...
model <- function(params){
  ...
  # with boost
  f.times.four.known <- sapply(1:reps, function(x){dependent.accumulators.with.boundary(4, p, end, boost, c(0,rep(floor(end*pre.k),3)), 100)[[1]]})
  
  # no boost
  f.times.four.unknown <- sapply(1:reps, function(x){dependent.accumulators.with.boundary(4, p, end, boost, c(0, 0, 0, 0), 100)[[1]]})
  
  # predict learning time in the one triple condition
  f.times.one <- sapply(1:reps, function(x){dependent.accumulators.with.boundary(1,p,end,0, c(0), 100)[[1]]})
...
result <- DEoptim(model, lower=c(0.00001,1,0,0), upper=c(1,1000,500,1), fnMap=mappingFun)
# empirical data ####
