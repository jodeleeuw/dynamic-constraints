...
data.c1 <- read.csv('data/explicit-debrief/coder-1.csv')
data.c2 <- read.csv('data/explicit-debrief/coder-2.csv')
...
sum(agree[,3:7])/ (5*259) * 100 # percent agreement
...
data.c3 <- read.csv('data/explicit-debrief/coder-3.csv')
...
data.merged.filter <- subset(data.merged, subject!=117)
